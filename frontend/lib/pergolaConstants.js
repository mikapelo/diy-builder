 ...
 *
 * Catégorie : product_preset — pergola autoportée, conformité DTU 31.1
 * appliquée aux principes de durabilité bois (la pergola n'est pas une
 * charpente couverte au sens strict du DTU mais en respecte les règles).
 *
 * Convention d'axes :
...
// ── Espacement (m) ──────────────────────────────────────────────
export const RAFTER_SPACING = 0.60;   // Entraxe chevrons (Eurocode 5 / DTU 31.1 + BILP)
export const MAX_POST_SPAN  = 3.50;   // Portée libre max entre poteaux (m) — flèche L/300 selon NF EN 1995-1-1

// ── Dimensions par défaut (m) ───────────────────────────────────
export const DEFAULT_HEIGHT = 2.30;   // Hauteur poteau sol→longeron (product_preset)
export const OVERHANG       = 0.15;   // Porte-à-faux chevron de chaque côté (NF DTU 31.1 §5.10.4.1)
...
// Références : plans professionnels standard, NF DTU 31.1 §5.7 (assemblages)
...
// ── Quincaillerie ───────────────────────────────────────────────
export const VIS_PER_RAFTER_BEAM = 2; // Vis D6×90 par intersection chevron/longeron (NF DTU 31.1 §5.7)
